...
class Upload_model extends MY_Model {
	...
	public function delete_wfile($key=FALSE)
	{
		if(!$key) return FALSE;
		$result = $this->uploadtoken->deleteFromQiNiu($key,$this->app_ini['bucket'],$this->app_ini['accessKey'],$this->app_ini['secretKey']);
		$this->db->delete("files",array("key"=>$key));
		return $result;
	}
	...
	public function sign_files(){
		...
		$result["baseUrl"] = $this->uploadtoken->makeBaseUrl($_key_,$this->app_ini['bucket'].'.qiniudn.com');
