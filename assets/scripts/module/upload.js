	...
	$(window).on('load', function () {
		...
		$('[data-upload]').each(function(){
			...
			var new MultipleUploader({
				...
				data: spy.data('upload-data') || {},
				multiple: true,
				error: function(file) {
					spy.trigger('upload.error',[file]);
				},
				success: function(response) {
					spy.trigger('upload.success',[response]);
				},
				progress: function(event, position, total, percent, files) {
					spy.trigger('upload.progress',[event, position, total, percent, files]);
				}
			})
		})
	})