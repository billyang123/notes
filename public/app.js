...
$('script[data-main][data-main!=""]:first').each(function() {
  return $.each($.trim($(this).data('main')).split(/[\s,]+/), function(i, m) {
    return require(m);
  });
});
});

;require.register("module/ajax", function(exports, require, module) {
var $document;
...
$document.on('ajax:error', function(evt, $xhr, status, error) {
  if ($xhr.responseText.length) {
    return $.UIkit.notify($xhr.responseText, {
      status: 'danger'
    });
  } else {
    return $.UIkit.notify('Something went wrong.', {
      status: 'danger'
    });
  }
});
...
;require.register("module/jquery-alert", function(exports, require, module) {
var $;

$ = jQuery;

$.alert = function(msg, status, pos) {
  if (status == null) {
    status = 'info';
  }
  if (pos == null) {
